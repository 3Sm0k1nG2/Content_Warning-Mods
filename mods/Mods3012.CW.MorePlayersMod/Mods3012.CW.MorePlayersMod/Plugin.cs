using BepInEx.Logging;
using BepInEx;
using HarmonyLib;

namespace Mods3012.CW.MorePlayersMod
{
    [BepInDependency(ForceSleepMod.PluginInfo.PLUGIN_GUID)]
    [ContentWarningPlugin(PluginInfo.PLUGIN_GUID, PluginInfo.PLUGIN_NAME, PluginInfo.PLUGIN_VANILLA_COMPATIBLE)]
    [BepInPlugin(PluginInfo.PLUGIN_GUID, PluginInfo.PLUGIN_NAME, PluginInfo.PLUGIN_VERSION)]
    [BepInProcess(ProcessInfo.PROCESS_NAME)]
    internal class Plugin : BaseUnityPlugin
    {
        internal static new ManualLogSource Logger { get; private set; }
        internal static PluginConfigEntries ConfigEntries { get; private set; }

        private Harmony harmony;


        private void Awake()
        {
            Logger = base.Logger;

            ConfigEntries = new PluginConfigEntries(Config);

            harmony = new Harmony(PluginInfo.PLUGIN_GUID);
            harmony.PatchAll();

            Logger.LogMessage($"Plugged-in successfully.");
        }

        private void OnDestroy()
        {
            harmony?.UnpatchSelf();

            Logger.LogMessage($"Unplugged succesfully");

            Logger = null;
            ConfigEntries = null;
            harmony = null;
        }
    }
}
